<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="退房编号" prop="tuifangbianhao">
            <el-input v-model="ruleForm.tuifangbianhao" 
                placeholder="退房编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tuifangbianhao" label="退房编号" prop="tuifangbianhao">
              <el-input v-model="ruleForm.tuifangbianhao" 
                placeholder="退房编号" readonly></el-input>
          </el-form-item>
        </div>
...
export default {
  ...
  created() {
	this.ruleForm.tuifangshijian = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='tuifangbianhao'){
            this.ruleForm.tuifangbianhao = obj[o];
	    this.ro.tuifangbianhao = true;
            continue;
          }
          if(o=='minsumingcheng'){
      ...
      this.$http({
		...
		if(json.shenfenzheng!=''&&json.shenfenzheng){
          ...
          this.$message.error(data.msg);
            ...
            this.dingdanzhuangtaiOptions = "未退房,已退房".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `wanchengdingdan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wanchengdingdan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wanchengdingdanCrossAddOrUpdateFlag = false;
						 this.parent.search();
